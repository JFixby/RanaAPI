...
// }
	...
	public List<ResourcesGroup> findBanks (final File assets_folder) throws IOException {
		...
		{
			final RedBank bank = this.findBank(assets_folder, RedResourcesManager.COLLECT_TANKS);
			if (bank != null) {
				result.add(bank);
				return result;
			}
		...
		for (final File file : assets_folder.listDirectChildren()) {
			...
			final RedBank bank = this.findBank(file, RedResourcesManager.COLLECT_TANKS);
			if (bank != null) {
				result.add(bank);
			}
		}
	...
	public ResourcesGroup installRemoteBank (final HttpURL bankUrl, final File assets_cache_folder, final Iterable<String> tanks)
		...
		final RedBank bank = this.findBank(httpRemote, !RedResourcesManager.COLLECT_TANKS);
		if (bank == null) {
